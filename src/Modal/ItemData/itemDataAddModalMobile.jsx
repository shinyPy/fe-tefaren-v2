  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (accessToken) {
          const response = await axios.get(
            "http://jp.tky.1.paladinsrv.com:25932/api/kategori-values",
            {
              headers: {
                Authorization: `Bearer ${accessToken}`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://jp.tky.1.paladinsrv.com:25932/api/barangAdd",
        formDataToSend,
        {
          headers: {
...
{validasiStep.find((message) => message.fieldName === "kode_barang") ? (
  ...
  <p className="mt-4 ml-0 text-red-700">Mohon pilih gambar terlebih dahulu</p>
                ...
                {!selectedImage && (
                  <label
                    for="file"
                    className={`relative text-xs mt-4 mb-4 flex min-h-[200px] tracking-widest1 items-center justify-center rounded-md border border-dashed border-[#e0e0e0] p-12 text-center border-2 ${
                      validasiForm
                        ? "border-red-500"
                        : ""
                    }`}
                ...
                {selectedImage && (
                  <div className=" mt-4">
                    <img
                      src={selectedImage}
                      alt="Preview"
                      className="min-h-[300px] max-h-[250px] w-full mb-4"
                    />
                  </div>
                )}
